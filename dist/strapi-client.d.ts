...
type StrapiUnifiedResponse<T> = {
    id: number | string;
    attributes: T;
};
type InferedTypeFromArray<T> = T extends Array<infer U> ? U : T;
type StrapiPopulatedResponse<T> = {
...
type Provider = 'auth0' | 'cas' | 'cognito' | 'discord' | 'email' | 'facebook' | 'github' | 'google' | 'instagram' | 'linkedin' | 'microsoft' | 'reddit' | 'twitch' | 'twitter' | 'vk';
type User = {
    id: number;
    username: string;
    email: string;
...
type CrudOperators = 'eq' | 'ne' | 'lt' | 'gt' | 'lte' | 'gte' | 'in' | 'notIn' | 'contains' | 'notContains' | 'containsi' | 'notContainsi' | 'between' | 'null' | 'notNull' | 'startsWith' | 'endsWith';
type RelationalFilterOperators = 'eq' | 'ne' | 'lt' | 'gt' | 'lte' | 'gte' | 'in' | 'notIn' | 'contains' | 'notContains' | 'startsWith' | 'endsWith';
type CrudFilter<T> = {
    field: keyof T;
